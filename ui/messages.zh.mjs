/** Auto-generated — do not edit. Source: shared/user-messages.zh.ts */
export const UI = {
  "productName": "Relay",
  ...
  "usageSummary": "验证许可证后，可为本机 Cursor 切换登录账号。关闭窗口可最小化到系统托盘（任务栏右下角图标），右键可退出；再次打开程序会聚焦已有窗口。换号有每日次数与间隔限制。",
...
export function reasonToChinese(reason) {
  return (function reasonToChinese(reason) {
    switch (reason) {
        case 'expired':
...
export function formatQuotaShort(quota) {
  return (function formatQuotaShort(quota) {
...
export function formatPlanCycleLine(opts) {
  return (function formatPlanCycleLine(opts) {
...
export function formatQuotaDetail(quota) {
  return (function formatQuotaDetail(quota) {
    ...
    if (quota.sessionRotatesMaxPerDay > 0 && !unifiedDaily) {
        parts.push(`今日重新打开可换新账号：剩 ${quota.sessionRotatesRemainingToday}/${quota.sessionRotatesMaxPerDay} 次`);
    }
    if (quota.nextSwitchAllowedAt) {
        const remain = Math.max(0, Math.ceil((Date.parse(quota.nextSwitchAllowedAt) - Date.now()) / 1000));
        if (remain > 0)
            parts.push(`请 ${remain} 秒后再换号`);
    }
    return parts.join('；');
})(quota);
}
...
export function formatExpiresAt(iso) {
  return (function formatExpiresAt(iso) {
    if (iso == null || iso === '')
        return '';
...
export function sessionStatusMessage(opts) {
  return (function sessionStatusMessage(opts) {
    if (opts.hubOffline)
        return UI.heroOffline;
    if (!opts.valid)
        return UI.heroIdle;
    ...
    if (opts.hasAccount)
        return UI.heroRestored;
    return UI.heroValid;
})(opts);
}

export function unwrapIpcErrorMessage(raw) {
  return (function unwrapIpcErrorMessage(raw) {
    const idx = raw.lastIndexOf('Error: ');
    if (idx >= 0)
        return raw.slice(idx + 'Error: '.length).trim();
    return raw.trim();
})(raw);
}

export function userFacingError(err) {
  return (function userFacingError(err) {
    ...
    function passThroughPipelineMessage(msg) {
    ...
    if (msg.includes('许可证') || msg.includes('请先验证'))
        return withSentencePeriod(msg);
